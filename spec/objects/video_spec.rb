...
describe Video do
  ...
  describe '#title' do
    it 'should go to the external API when there is no title persisted' do
      expect(described_instance.title).to eq title_for_id
    end
    
    it 'should not return anything when the video has no title' do
      build(:video, id: id, source: source_name)
    ...
    it 'should should return a title for a video when one is persisted' do
      ...
      create(:title, title: title, video: video_object)

      expect(described_instance.title).to eq title
    end
  end
end
